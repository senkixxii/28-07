...
export type BearMood = 'happy' | 'sleepy' | 'wink' | 'party' | 'sad' | 'wave' | 'hug'
...
export default function BearMascot({ size = 96, mood = 'happy', float = false, className, easterEgg = false }: BearMascotProps) {
  ...
  // Occasional gentle blink — skipped for moods with their own eye shapes and under reduced motion.
  useEffect(() => {
    if (reducedMotion || ['sleepy', 'wink', 'sad'].includes(activeMood)) return
  ...
  const svg = (
    ...
    >
      ...
      {blinking && !['sleepy', 'wink', 'sad'].includes(activeMood) ? (
        <>
          <path d="M42 62 Q48 65 54 62" stroke="#4A3F45" strokeWidth="3" strokeLinecap="round" fill="none" />
          <path d="M66 62 Q72 65 78 62" stroke="#4A3F45" strokeWidth="3" strokeLinecap="round" fill="none" />
        </>
      ) : activeMood === 'sleepy' ? (
        <>
        ...
        <>
          <path d="M45 60 Q48 64 52 60" stroke="#4A3F45" strokeWidth="3" strokeLinecap="round" fill="none" />
          <path d="M68 60 Q72 64 75 60" stroke="#4A3F45" strokeWidth="3" strokeLinecap="round" fill="none" />
        </>
      ) : (
        <>
          <circle cx="48" cy="62" r="3.6" fill="#4A3F45" />
      ...
      {activeMood === 'party' || activeMood === 'happy' || activeMood === 'wave' || activeMood === 'hug' ? (
      ...
      {activeMood === 'wave' && (
        <motion.g
          animate={reducedMotion ? undefined : { rotate: [0, 22, 0, 22, 0] }}
          transition={{ duration: 1.1, repeat: 1, ease: 'easeInOut' }}
          style={{ transformOrigin: '96px 62px' }}
        >
          <circle cx="96" cy="52" r="11" fill="#FBCBDD" />
          <circle cx="96" cy="52" r="11" fill="url(#bear-shade)" opacity="0.4" />
        </motion.g>
      )}

      {activeMood === 'hug' && (
        <>
          <ellipse cx="38" cy="88" rx="10" ry="8" fill="#FBCBDD" transform="rotate(-25 38 88)" />
          <ellipse cx="82" cy="88" rx="10" ry="8" fill="#FBCBDD" transform="rotate(25 82 88)" />
          <motion.text
            x="60"
            y="100"
            textAnchor="middle"
